...
const { ProofType, zkVerifySession, Library, CurveType } = require("zkverifyjs");
...
async function getInitialNonce(session, account, proofType) {
...
async function getNextNonce(proofType) {
    return nonceMutex.runExclusive(async () => {
        if (nonceTracker[proofType] === undefined) {
...
function selectProofExecution(session, proofType, nonce) {
    switch (proofType) {
...
async function sendProofTransaction(proofType) {

...
class WalletPool {
    ...
    async acquireWallet(): Promise<[string, string]> {
        try {
            const response = await axios.get<{ key?: string; wallet?: string }>(`${this.walletServerUrl}/wallet`, { timeout: 61000 });

            if (!response.data.key || !response.data.wallet) {
                throw new Error("Invalid response: Wallet data missing");
            }

            return [response.data.key as string, response.data.wallet as string];
        } catch (error: unknown) {

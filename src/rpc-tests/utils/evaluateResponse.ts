...
function testString(value: any, pattern: any): void {
...
function testNumber(value: any): void {
  log(`Testing number: Value = ${value}`);
  expect(value).not.toBeNaN();
}

function testNull(value: any): void {
  log(`Testing for null: Value = ${value}`);
  expect(value).toBeNull();
}

function testBoolean(type: any): void {
  expect(type).toBe("boolean");
}

function unsupportedType(type: any): void {
  throw new Error(
      `Unsupported type. Expected "array", "object", "string", "number", "null", or "boolean" but received: "${type.toLowerCase()}".`
  );
}

function iterateArrayItems(value: any, pattern: any): void {
...
function iterateObjectProperties(value: any, pattern: any): void {
  for (const key in value) {
    reduceValue(value[key], (pattern && pattern[key]) || null);
  }
}

function reduceValue(value: any, pattern: any): void {
...
interface EvaluateResponseParams {
  response: Response;
  pattern: any;
  expectNullResult?: boolean;
}

function evaluateResponse({ response, pattern, expectNullResult = false }: EvaluateResponseParams): void {
